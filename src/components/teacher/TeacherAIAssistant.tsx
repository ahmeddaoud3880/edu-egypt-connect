...
const QUICK_QUESTIONS = {
  ar: ["ما هى درجات الفصل الدراسى؟", "من هم الطلاب الضعاف؟", "كيف حضور الطلاب اليوم؟", "اعطنى ملخص للفصل"],
  en: ["What are the class grades?", "Who are the weak students?", "How is attendance today?", "Give me a class summary"],
};

export function TeacherAIAssistant() {
  const { lang } = useTranslation();
  const isAr = lang === "ar";
  const { messages, loading, error, sendMessage, clearMessages } = useAIChat("teacher");
      ...
      <div className="bg-surface-elevated rounded-lg border border-border p-5">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-semibold text-foreground">{isAr ? "المساعد الذكى للمعلم" : "AI Teaching Assistant"}</h2>
              <p className="text-xs text-muted-foreground">{isAr ? "تحليل الفصول والطلاب بيانات حقيقية" : "Real-time class and student analysis"}</p>
            </div>
          </div>
          {messages.length > 0 && <button onClick={clearMessages} className="text-xs text-muted-foreground hover:text-destructive flex items-center gap-1"><Trash2 className="w-3 h-3" /></button>}
        </div>
      </div>
        ...
        <div className="h-96 overflow-y-auto p-4 space-y-4">
          {messages.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full gap-4 text-center">
              ...
              <p className="text-sm text-muted-foreground">{isAr ? "اسأل عن فصلك أو طلابك" : "Ask about your class or students"}</p>
              <div className="flex flex-wrap gap-2 justify-center">
                {QUICK_QUESTIONS[isAr ? "ar" : "en"].map(q => <button key={q} onClick={() => sendMessage(q)} className="text-xs px-3 py-1.5 bg-primary/10 text-primary rounded-full hover:bg-primary/20">{q}</button>)}
              </div>
            </div>
          )}
          {messages.map(msg => (
          ...
          {loading && <div className="flex justify-start"><div className="bg-muted rounded-2xl px-4 py-3 flex items-center gap-2"><Loader2 className="w-4 h-4 animate-spin text-muted-foreground" /><span className="text-xs text-muted-foreground">{isAr ? "يفكر..." : "Thinking..."}</span></div></div>}
