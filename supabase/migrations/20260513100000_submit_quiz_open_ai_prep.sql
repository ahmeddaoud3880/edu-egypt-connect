-- Open-ended: defer points to AI regrade endpoint (exact string match removed here).
-- Return submission row id for follow-up grading. Optional grading_details for AI / teacher overrides.
...
BEGIN
  ...
  LOOP
    ...
    ELSIF rec.question_type = 'open' THEN
      -- Deferred: graded by Egypt.edu AI agent (compare to model answer). Counts as 0 here.
      ok := false;
    END IF;
...
COMMENT ON FUNCTION public.submit_student_quiz(uuid, jsonb) IS
  'MCQ/TF auto-score; open-ended deferred to AI regrade; returns submission_id.';
